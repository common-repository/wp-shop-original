...
require_once(WPSHOP_CLASSES_DIR.'/sofort_lib/core/sofortLibNotification.inc.php');
require_once(WPSHOP_CLASSES_DIR.'/sofort_lib/core/sofortLibTransactionData.inc.php');
$sofort_set = get_option("wpshop.payments.sofort");
...
if($SofortLibTransactionData->isError()) {
	...
	if($SofortLibTransactionData->getStatus()=='received'){
		global $wpdb;
		$wpdb->query("DELETE FROM {$wpdb->prefix}wpshop_selected_items WHERE selected_items_session_id='".$SofortLibTransactionData->getUserVariable(0)."'");
		$order_id = $SofortLibTransactionData->getReason();
		Wpshop_Orders::setStatus(intval($order_id),1);
	} 
	if($SofortLibTransactionData->getStatus()=='untraceable'){
		global $wpdb;
		$wpdb->query("DELETE FROM {$wpdb->prefix}wpshop_selected_items WHERE selected_items_session_id='".$SofortLibTransactionData->getUserVariable(0)."'");
		$order_id = $SofortLibTransactionData->getReason();
		Wpshop_Orders::setStatus(intval($order_id),6);
	} 
}
